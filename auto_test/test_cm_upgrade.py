...
def selectArticle(cur):
    sql = "select * from article where status = 0 ORDER BY create_time DESC limit 100"
    cur.execute(sql)
    result = cur.fetchall()
    a = []
    if len(result) > 100:
...
def permission(cur,uid):      #用户兑换积分的权限（查询）
    sql = "select * from user_permission WHERE user_id = '%d' and (pid = 1 or pid = 4)" % (uid)
    cur.execute(sql)
    result = cur.fetchall()
    if(len(result)) == 0:
        print ("该用户无积分兑换权限，现增加他的权限，等待2s....")
        return 0
    else:
        return 1

def insert_permission(conn,cur,uid,perm):     #增加用户权限
    ...
    if perm == 'exchange':
        ...
        print '输入的perm参数值与数据库perm字段的值不同，请注意！'
    sql2 = "insert into user_permission VALUE('%d', '%d', '%s', '%s', 0, now(), now()) " % (id,pid,perm,uid)
    cur.execute(sql2)
    conn.commit()

def delete_permission(conn, cur, uid,perm):   #删除用户兑换积分权限
    sql = "delete  from user_permission WHERE user_id = '%d' and perm_name = '%s' " % (uid,perm)
    cur.execute(sql)
    conn.commit()
...
def select_match(cur):
    ...
    b = datetime.datetime.strptime(a,'%Y-%m-%d %H:%M:%S')       #再将string类型的日期转换成datetime类型，主要是为了能够将日期进行加减操作
...
def select_live_match(cur):                                 #Live页面显示的数据查询，直播比赛超过10分钟后会被删除
    sql1 = "select * from `match` WHERE start_play > now() and status = '2' or status = 1 ORDER by start_play "     #如果查询的时候正好有比赛在直播时的情况,所以需要
    cur.execute(sql1)
    result1 = cur.fetchall()
    start_time = result1[0][20]
    end_time = start_time + datetime.timedelta(days=1)
    sql2 = "select * from `match` WHERE  start_play >= '%s' and start_play <= '%s' and status = '2' or status = 1" \
           " ORDER by start_play " % (start_time, end_time)
    cur.execute(sql2)
    result2 = cur.fetchall()
    b = []
    for i in range(len(result2)):
        now = datetime.datetime.now()
        now1 = now.strftime('%Y-%m-%d %H:%M:%S')  # 将datetime类型的日期转换成string类型，主要是为了去掉秒数后的小数点位数
        now2 = datetime.datetime.strptime(now1, '%Y-%m-%d %H:%M:%S')  # 再将string类型的日期转换成datetime类型，主要是为了能够将日期进行加减操作
        rs_time = now2 - datetime.timedelta(minutes=10) - datetime.timedelta(hours=8)
        # print result2[i][1],
        if result2[i][13] == 1 and   result2[i][20]  < rs_time:                    #现在的时间减去10分钟后的时间，如果该时间大于比赛开始时间，那么该比赛已开始了10分钟以上
            print 'match_id为' + str(result2[i][1]) + '的比赛正在直播，但10分钟内无直播数据，故已删除'
        else:
            b.append(result2[i][1])
    return b
...
def youtube_video(cur):
    a = datetime.datetime.now()
    b = a.strftime('%Y-%m-%d %H-%M-%S')
    date = datetime.datetime.strptime(b, '%Y-%m-%d %H-%M-%S')
    date1 = date - datetime.timedelta(hours=date.hour) - datetime.timedelta(minutes=date.minute) - datetime.timedelta(seconds=date.second)
    date2 = date1 - datetime.timedelta(days=3)
    sql = 'select url from video where "%s" < create_time and create_time < "%s"' % (date2,date1)
    a = []
    cur.execute(sql)
    result = cur.fetchall()
    for i in range(len(result)):
...
def select_match_prediction(cur):                          #查询竞猜页面中的比赛，查询规则是先找出距离现在最近的一场比赛，然后再根据找到的比赛找距该比赛7天内的所有比赛
    sql1 = "select * from `match` WHERE leg_name = 'EPL' and start_play > now() and status = '2' ORDER by start_play"
    cur.execute(sql1)
    result1 = cur.fetchall()
    start_time = result1[0][20]
    end_time = start_time + datetime.timedelta(days=7)
    sql2 = "select * from `match` WHERE leg_name = 'EPL' and  start_play >= '%s' and start_play <= '%s' and status = '2'" \
           " ORDER by start_play "% (start_time,end_time)
    cur.execute(sql2)
    result2 = cur.fetchall()
    b = []
    for i in range(len(result2)):
        # print result2[i][1],                                  #逗号可以使输出不换行，该行隐藏掉，不然显示会有问题，显示字符太长，显示不全后面的列表
        b.append(result2[i][1])
    return b
