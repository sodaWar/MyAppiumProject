...
def signHeaders(payload,request_data_type):
...
def interfaceTest(num, api_purpose, api_host, request_url, request_method,request_data_type,request_data, encryption,check_point):
    ...
    headers = signHeaders(payload,request_data_type)
...
def main():
    ...
    if len(errorTest) > 0:
        html = '<html><body>接口自动化定期扫描，共有 ' + str(len(errorTest)) + ' 个异常接口，列表如下：' + '</p><table><tr><th style="width:10px;">接口</th><th style="width:10px;">状态</th><th style="width:200px;">接口地址</th><th>接口返回值</th></tr>'
